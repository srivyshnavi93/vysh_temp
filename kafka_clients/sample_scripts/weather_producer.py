...
def find_weather(city_name):
   ...
   try:
    ...
    #    location = soup.select('#wob_loc')[0].getText().strip()
       ...
       humidiy = soup.select('#wob_hm')[0].getText().strip()
...
kafka_producer = KafkaProducer(bootstrap_servers = ['kafka-broker:9092'], value_serializer = lambda x:dumps(x).encode('utf-8'))
